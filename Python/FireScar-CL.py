...
table = ee.FeatureCollection("users/italomoletto/MAPAS/FireDB_1985-2018_31052019_latlon").filterMetadata('area','greater_than',9)
...
#l=['ID','start_fire','end_fire','conaf_area','model_area','model_type']
#append_list_as_row(r'C:\Users\Administrador\Documents\mole3/FireSCAR_data.csv', l)

for i in range(10259,lista_for):
                print(i)
                ...
                if not pref.getInfo()['bands']:
                    print('pre fire image not found')
                    continue
                if not postf.getInfo()['bands']:
                    print('post fire image not found')
                    continue
                ##############/--- BOX 4 ---###########################/ 
                ################################################/
                ################################################/
                ...
                image_export = mosaic_pre.select(['B2','B3','B4','B5','B6','B7']).addBands(mosaic_pos.select(['B2','B3','B4','B5','B6','B7'])).addBands(dNBR).addBands(ndvipre).addBands(ndvipos)
                #minimal value 5
                percentile = 90
                ...
                try:
                    RdNBR_value = ind_RdNBR.getInfo()['bands'][0]['data_type']['max']
                except ee.EEException:
                    print('no value for RdNBR found')
                    continue
                RdNBR_value_round=round(RdNBR_value,2)
                RdNBR_value_round_str = str(RdNBR_value_round).replace('.','-')
                ...
                fire_scar_img_value = fire_scar_img.multiply(ind_RdNBR)
                ...
                bufsel0 = inc2.buffer(1000);  
                firevect_poly_area_b5f_buf0 = firevect_poly_area_b5f.filterBounds(bufsel0);
                firevect_area_clip = firevect_area#.clip(inc2)
                #     firevect_poly_area_b5f_buf0 = firevect_area.filterBounds(bufsel0)
                #    Map.addLayer(firevect_ndvi_area_b5f_aguaf_buf0)
                # Filtrar por distncia al incendios mas grnade cercano al centro
                maxArea = firevect_poly_area_b5f_buf0.sort('sum', False).limit(1).geometry(); # obtener poligono de mayor superficie
                ...
                area_first_filt  = area2.getInfo()['sum']
                area_second_filt = area3.getInfo()['sum']
                actual_fire = inc.getInfo()['properties']['area']*10000
                print('actual fire: '+str(actual_fire)+' square meters')
                conditional_fire=takeClosest(actual_fire,[area_first_filt,area_second_filt])
                if conditional_fire == area_second_filt:
                    print('using second filter as closest value: '+str(area_second_filt)+' squared meters')
                    scar_fire_model = fire_scar_vect_filtered_filt2_areabuf
                    model_area = area_second_filt
                    model_type = 'second_filter'
                else:
                    print('using first filter as closest value: '+str(area_first_filt)+' squared meters')
                    scar_fire_model = firevect_poly_area_b5f_buf
                    model_area = area_first_filt
                    model_type = 'first_filter'
                fire_scar_rast = fire_scar_img.clip(scar_fire_model)
                image_export = image_export.addBands(fire_scar_rast.rename('FireMask')).addBands(RdNBR).float()
                ##############/--- BOX 7 ---###########################/ 
                ################################################/
                ################################################/
                ...
                export = ee.batch.Export.image.toDrive(\
                                                       image=image_export,
                                                       description= id +'_'+ str(scar)+'_'+RdNBR_value_round_str+ '_Date_' + fin, #+ '_u' + ind_RdNBR,
                ...
                export = ee.batch.Export.table.toDrive(\
                                                       ...
                                                       description= id+'_'+RdNBR_value_round_str+'_' + str(scar) + '_Date_' + fin,# + '_u' + ind_RdNBR,
                                                       folder= 'FireScar-CL_'+fin[0:4],
                                                       fileFormat= 'SHP')
                export.start()
                l_export = [scar,inicio,fin,actual_fire,model_area,model_type]
                append_list_as_row(r'C:\Users\Administrador\Documents\mole3/FireSCAR_data_logs.csv', l_export)