...
th_ndvi1 = 0.1
th_ndvi2 = 0.1
#for th_nbr in range(0.05,0.15,0.01):
while th_nbr < 0.16:
   # for th_ndvi1 in range(0.10,0.3,0.01):
   while th_ndvi1 < 0.3:
        #for th_ndvi2 in range(0.10,0.3,0.01):
        while th_ndvi2 < 0.3:
            ###@@ --------------------------------------Comienzo del codigo que entra al loop ----------------------------------------@@###
            for i in range(79,lista_for):
                print(i)
                ...
                inicio = str(int(anoi.getInfo())) + '-' + str(int(mesi.getInfo())) + '-' + str(int(diai.getInfo()))
                fin    = str(int(anof.getInfo())) + '-' + str(int(mesf.getInfo())) + '-' + str(int(diaf.getInfo()))
                """
                ...
                ndvipos = postf.normalizedDifference(['B5','B4']);
                ndvipre = pref.normalizedDifference(['B5','B4']);
                ...
                pref_nbr = pref.expression('float(b("'+B4+'") - b("'+B7+'")) / (b("'+B4+'") + b("'+B7+'"))');
                postf_nbr = postf.expression('float(b("'+B4+'") - b("'+B7+'")) / (b("'+B4+'") + b("'+B7+'"))');
                ...
                firesev = pref_nbr.addBands(postf_nbr);
                RdNBR = firesev.expression("(b('B5') - b('B5_1'))/(sqrt(abs(b('B5')/1000)))").toFloat().rename('RdNBR'); # metodo anterior
                #minimal value 5
                percentile = 90
                ...
                dwl = RdNBR.addBands(firesev).addBands(ndvipre).addBands(ndvipos);
                      #print(dwl);
                """
                export = ee.batch.Export.image.toDrive(\
                                                       image=dwl,
                                                       description= id + str(scars) + '_Date_' + inicio, #+ '_u' + ind_RdNBR,
                ...
                export = ee.batch.Export.table.toDrive(\
                                                       ...
                                                       description= id + str(scars).replace('.','') + '_Date_' + fecha,# + '_u' + ind_RdNBR,
                                                       folder= 'FireScars_thndvi1_'+str(th_ndvi1).replace('.','')+'_thndvi2_'+str(th_ndvi2).replace('.','')+'_thnbr_'+str(th_nbr).replace('.','')   +'_br',
                                                       fileFormat= 'KML')
                export.start()
            th_ndvi2 = th_ndvi2 + 0.01
        th_ndvi1 = th_ndvi1 + 0.01
   th_nbr = th_nbr + 0.01